...
def create_dash_app(**kwargs):
    ...
    subscriber = RawDataSubscriber(
        config_path=processed_obj.path_dict["configpath"],
        formulae_path=processed_obj.path_dict.get("formulaepath", ""),
        max_seconds=600,
        max_ui_hz=5,
    )
    ...
    def serve_layout():
        ...
        interval = dcc.Interval(id="refresh", interval=1 * 1000, n_intervals=0)
    ...
    def decimate_for_plot(x, y, max_points=2000):
        x = np.asarray(x)
        y = np.asarray(y)
    ...
    def refresh_graphs(intervals, plots):
        """
        Update dashboard plots from live subscriber buffer.

        This does NOT:
        - reread parquet
        - call PostProcessData.UpdateData()
        - write postprocessed parquet
        - reprocess the full data file

        It only:
        - gets latest rolling processed snapshot from subscriber
        - updates Plotly figures at the Dash interval rate
        """

        # Current figures from all graph-containing plot layouts.
        # Home layout usually has no Graph, so skip it.
        current_figures = []
        graph_plot_names = []

        for plot_children in plots:
            if not plot_children:
                continue

            first_child = plot_children[0]
            props = first_child.get("props", {})

            if "figure" not in props:
                continue

            graph_id = props.get("id", {})
            plot_name = graph_id.get("index")

            current_figures.append(props["figure"])
            graph_plot_names.append(plot_name)

        # Get latest live processed data from subscriber.
        snapshot = subscriber.snapshot()

        # If no live data has arrived yet, return current figures unchanged.
        if not snapshot or "Time" not in snapshot:
            return current_figures

        time_data = snapshot["Time"]

        df = processed_obj.All_chart_info.sort("Chart")

        updated_figures = []

        for current_figure, plot_name in zip(current_figures, graph_plot_names):
            fig = go.Figure(current_figure)

            # Clear old traces.
            fig.data = []

            # Rows/channels that belong to this chart.
            chart_rows = df.filter(pl.col("Chart") == plot_name)

            for row in chart_rows.iter_rows(named=True):
                label = str(row["Label"]).strip()
                legend = str(row["Legend"]).strip()
                position = int(row["Position"])

                if label not in snapshot:
                    continue

                y_data = snapshot[label]

                x_plot, y_plot = decimate_for_plot(
                    time_data,
                    y_data,
                    max_points=2000,
                )
            ...
            # Keeps zoom/pan state stable across refreshes.
            fig.update_layout(uirevision=plot_name)
