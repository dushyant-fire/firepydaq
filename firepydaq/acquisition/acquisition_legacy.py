    ...
    def runpyDAQ(self):
        ...
        if (samplesAvailable >= no_samples):
            try:
                ...
                if hasattr(self, "raw_publisher"):
                    data = np.asarray(self.ydata_new, dtype=np.float64)
                    if data.ndim == 1:
                        data = data.reshape(1, -1)

                    self.raw_publisher.publish(
                        {
                            "time": np.asarray(self.xdata_new),
                            "absolute_time": list(self.abs_timestamp),
                            "labels": list(self.labels_to_save),
                            "data": data,
                            "sample_rate": float(self.ActualSamplingRate),
                        }
                    )
